...
{
    public class FaqServiceFacade : IFaqServiceFacade
    {
        ...
        public FaqServiceFacade(IMapper mapper, IFaqService service, IUnitOfWork unitOfWork, IWebHostEnvironment webHostEnvironment, IConfiguration configuration)
        {
            _mapper = mapper;
            _service = service;
        ...
        public async Task<Result<FaqViewModel>> GetDefaultModelAsync()
        {
            try
            {
                ...
                if (result.IsSucceed)
                {
                    var model = _mapper.Map<FaqViewModel>(result.Data);
                    return Result<FaqViewModel>.Succeed(model);
                }
                return Result<FaqViewModel>.Succeed(new FaqViewModel());
            }
            catch (ApplicationException ex)
            {
                return Result<FaqViewModel>.Failure(ex.Message);
            }
            catch (Exception e)
            {
                return Result<FaqViewModel>.Failure(e);
            }
        }

        public async Task<Result> SaveAsync(FaqViewModel model)
        {
